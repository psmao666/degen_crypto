...
public:
    ...
    template<typename ResponseType>
    std::optional<ResponseType> post(const std::string& host, const std::string& target, const std::string& body, const std::string& api_secret, const std::unordered_map<std::string, std::string>& headers = {}, bool need_signature = false) {
        if (!need_signature) return post_helper<ResponseType>(host, target, body, headers);
        ...
        return post_helper<ResponseType>(host, new_target, body, headers);
    }
...
private:
    ...
    template<typename ResponseType>
    std::optional<ResponseType> post_helper(const std::string& host, const std::string& target, const std::string& body, const std::unordered_map<std::string, std::string>& headers = {}) {
        std::lock_guard<std::mutex> lock(https_mutex_);
        try {
            boost::asio::ip::tcp::resolver resolver(ioc_);
            const auto& results = resolver.resolve(host, "https");
